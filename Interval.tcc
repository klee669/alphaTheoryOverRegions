...
namespace homotopy
{
    ...
    Interval::Interval(const Type &value)
    {
        convert(upper,value);
        convert(lower,-value);
    }
    ...
    // Forward declaration to avoid issues with MPFR's version
    long exponent(double);
    
    // Get exponent of Interval (mantissa is assumed to be between 1/2 and 1)
    // Versions for the largest and smallest
    inline long max_exponent(Interval &interval)
    {
        return (long)std::max(exponent(interval.lower),exponent(interval.upper));
    }
    
    inline long min_exponent(Interval &interval)
    {
        return (long)std::min(exponent(interval.lower),exponent(interval.upper));
    }
    ...
    bool is_finite(const Interval &interval)
    {
        return ((std::isfinite(interval.lower)!=0) && (std::isfinite(interval.upper)!=0));
    }
}
